...
from .traffic_light import detect_red, detect_green, detect_green_liane
...
class LightDetector(Node):
    ...
    def timer_callback(self):
        ...
        green_light, image = detect_green_liane(np.asarray(image[:, :]))
        pub = Bool()
        pub.data = green_light
        self.publisher.publish(pub)
        if green_light:
            self.get_logger().info(f"Green")
        else:
            self.get_logger().info(f"NOT green")
    ...
    def callback(self, img_msg):
        ...
        # LIANE
        green_light, debug_image = detect_green_liane(np.asarray(image[:, :]))
        pub = Bool()
        pub.data = green_light
        self.publisher.publish(pub)
        if green_light:
