...
class PurePursuit(Node):
    ...
    def __init__(self):
        ...
        self.traffic_locations = np.array([[-10.8, 15.8],
                                            [-28.0, 34.1],
                                            [-54.8, 24.1]])
        self.traffic_radius = 2 # meters
    ...
    def near_traffic_light(self, x, y):

        pos = np.array([x, y])
        delta = self.traffic_locations - pos
        dists = np.linalg.norm(delta, axis=1)

        close = dists <= self.traffic_radius
    ...
    def pose_callback(self, odometry_msg):
        ...
        if time.time() - self.traffic_cooldown > 10\
            and self.near_traffic_light(car_pos_x, car_pos_y):
            if not self.is_near_traffic:
                self.get_logger().info("Traffic Light Zone")
                self.is_near_traffic = True
                self.traffic_stop = True
                msg = Bool()
                msg.data = True
                self.traffic_zone_pub.publish(msg)
                self.traffic_timer = time.time()
                # self.park(1)
                return
        else:
            if self.is_near_traffic:
                self.traffic_cooldown = time.time()
            self.is_near_traffic = False
        ...
        if (closest_segment_index + 1 == len(traj_x) - 1 and \
            ...
            if self.follow_shell:
                ...
                # closest_point = self.find_closest_point(car_pos_x, car_pos_y)
                # if np.linalg.norm(closest_point - np.array([car_pos_x, car_pos_y])) > 0.75:
                # reverse_time = np.linalg.norm(np.array(self.shell_traj.points[0]) - np.array(self.shell_traj.points[1])) - 
        ...
        if self.traffic_stop: 
            if time.time() - self.traffic_timer < 30: 
                drive_msg = AckermannDriveStamped()
                drive_msg.drive.speed = 0.25
                # steer_angle = self.real_car_angle_offset
                # self.drive_pub.publish(drive_msg)
            else:
                green_msg = Bool()
                green_msg.data = True
